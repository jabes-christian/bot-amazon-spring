...
>
> **Nota de execução (2026-09-08, durante T4)**: o `ExecutorService` de threads virtuais **não pode** ser criado por chamada via `try`-with-resources. `ExecutorService.close()` (Java 21) chama `shutdown()` e depois **bloqueia a thread chamadora** até a tarefa em andamento terminar (só força `shutdownNow()` se a própria thread chamadora for interrompida enquanto espera) — ou seja, fechar o executor ao sair do bloco anularia o próprio timeout: uma chamada ao LLM travada faria `gerarCopy` bloquear pelo tempo real da chamada, não pelo timeout configurado, exatamente o cenário que ENRICH-04 existe para evitar. Corrigido: o executor é um campo de longa duração de `CopyGenerationService` (criado uma vez, nunca fechado por `gerarCopy`). Threads virtuais são daemon por definição da JVM, então isso não impede o shutdown da aplicação nem vaza recursos de forma problemática.
