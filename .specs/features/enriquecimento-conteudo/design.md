...
`OpenAiChatModel` do LangChain4j aceita um `.timeout(Duration)` no builder do bean — mas isso fixaria o valor na construção do bean (só mudaria com restart), contrariando AD-009, que já lista explicitamente "timeout do LLM" como um limiar de negócio que deve ser ajustável sem redeploy. Decisão: o bean `ChatModel` **não** define timeout no builder; `CopyGenerationService` lê `enriquecimento.llm-timeout-segundos` do `ConfigService` a cada chamada e aplica o timeout na própria chamada, via `CompletableFuture.supplyAsync(() -> chatModel.chat(prompt), Executors.newVirtualThreadPerTaskExecutor()).get(timeout, TimeUnit.SECONDS)` (threads virtuais do Java 21 — sem biblioteca nova, sem pool para gerenciar). Qualquer exceção (timeout, erro de API, resposta vazia) cai no mesmo `catch` e aciona o fallback por template.
...
| Feature `canais-disparo` | Chamada de método Java a `EnriquecimentoService.enriquecer(produto)`; e, após concluir todos os canais de um produto, `canais-disparo` **deve** chamar `BannerImageService.removerBanner(path)` — contrato que o design de `canais-disparo` precisa honrar (ENRICH-08 AC4) |
...
| Timeout do LLM: builder do LangChain4j vs. enforcement manual por chamada | Enforcement manual via `CompletableFuture` + `Executors.newVirtualThreadPerTaskExecutor()`, lendo o valor do `ConfigService` a cada chamada | Único jeito de honrar AD-009 (timeout ajustável sem redeploy) — o builder do `OpenAiChatModel` fixaria o valor na construção do bean |
