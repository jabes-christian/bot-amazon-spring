...
- **Interfaces**:
  - `ConteudoEnriquecidoDTO enriquecer(Product produto): ConteudoEnriquecidoDTO` — chama `copyGenerationService.gerarCopy(produto)` (retorna `CopyResultadoDTO`) e `bannerImageService.gerarBanner(produto)`, monta `new ConteudoEnriquecidoDTO(resultado.texto(), bannerPath.orElse(null), resultado.viaLlm())`
...
- **Interfaces**:
  - `Optional<Path> gerarBanner(Product produto): Optional<Path>` — baixa a imagem via `RestClient`; se falhar ou o conteúdo não for uma imagem válida (`ImageIO.read` retorna `null`), retorna `Optional.empty()` (sinaliza modo texto puro, ENRICH-08 AC3); senão compõe o banner via Java2D e grava em arquivo temporário **no formato e resolução fixados abaixo**, retornando seu `Path` (sempre extensão `.jpg`)
