...
| `EnriquecimentoService.enriquecer(CandidatoPromocaoDTO)` / `BannerImageService.removerBanner(Path)` | `service/` (feature `enriquecimento-conteudo`) | Consumidos exatamente conforme o contrato já fechado (Integration Point daquele design) — assinatura corrigida em 2026-09-10 para bater com a implementação real de `enriquecimento-conteudo` (ver `EnriquecimentoService.java:16`); esta tabela estava desatualizada desde a Revisão 2 registrada abaixo em `DisparoService`, que já usava a assinatura correta |
...
**Nota de atenção para o Verifier (registrada em 2026-09-10, revisão pré-Tasks)**: `DisparoService` é o componente mais denso deste design — 6 passos, 2 revisões de contrato já absorvidas, 8 dependências injetadas. É o alvo mais provável de gap de cobertura quando o Verifier rodar após a implementação (mesmo padrão que `BannerImageService` foi em `enriquecimento-conteudo`, onde 2 iterações de fix→re-verify foram necessárias). Nenhuma mudança de design decorre disso agora — é só um alerta para dar atenção redobrada aos testes desta classe específica durante Execute (múltiplos canais por produto, múltiplos produtos por canal, dedup dentro/fora da janela, retry, contagem ENRICH-07) e para o Verifier considerar um sensor de discriminação mais generoso que o mínimo de 1-3 mutações nesta classe, dado o risco concentrado.

### `DisparoScheduler`

- **Purpose**: Disparar `DisparoService.executarCicloDisparo()` no cron configurado, garantindo que uma nova execução nunca sobreponha uma anterior ainda em andamento (DISPATCH-08).
- **Location**: `scheduler/DisparoScheduler.java`
- **Interfaces**: `@Scheduled(cron = "${disparo.cron}", zone = "America/Sao_Paulo") void executarCicloAgendado(): void`
- **Dependencies**: `DisparoService`
- **Lógica**:
  1. Um campo `AtomicBoolean emExecucao` (inicial `false`), escopo de instância do bean singleton (Spring `@Component` default), guarda o estado de "ciclo em andamento" entre disparos do cron.
  2. No início de `executarCicloAgendado`: `if (!emExecucao.compareAndSet(false, true))` → loga WARN ("ciclo de disparo anterior ainda em execução, pulando esta execução agendada") e retorna sem chamar `DisparoService` (DISPATCH-08).
  3. Se a troca atômica teve sucesso: chama `disparoService.executarCicloDisparo()` dentro de um `try`, e libera a flag (`emExecucao.set(false)`) num `finally` — garantindo que uma exceção não deixada o ciclo "travado" como sempre em execução.
- **Reuses**: Nenhum — decisão de lock local descrita abaixo em Tech Decisions
...
| A interpretação de "HTTP 201 + corpo `{status: PENDING}` = sucesso" da Evolution API (ver `WhatsAppChannelSender` acima) é uma decisão documentada, mas nunca validada contra uma instância real da Evolution API — nenhuma chamada de verdade foi feita nesta sessão | `WhatsAppChannelSender` (a ser criado) | Se a Evolution API real não seguir exatamente esse formato, o sender trataria uma falha real como sucesso (ou vice-versa), corrompendo silenciosamente o dedup (`DispatchHistory` gravado indevidamente) | **Registrado em 2026-09-10, a pedido do usuário**: a task de `WhatsAppChannelSender` deve mockar exatamente esse cenário (HTTP 201 com corpo `{"status":"PENDING", ...}`), não um HTTP 200 genérico — o teste precisa provar que o sender trata esse formato específico como sucesso, já que é a interpretação documentada aqui, não a única fisicamente possível. Validação contra uma instância real da Evolution API fica fora do escopo do Execute (sem instância disponível neste ambiente) — risco residual aceito, mas o teste deve ao menos refletir fielmente a decisão tomada aqui |
...
| Mecanismo de lock contra overlap do scheduler de disparo (DISPATCH-08) | `AtomicBoolean` de instância no `DisparoScheduler` (`compareAndSet` no início, liberado em `finally`) — não um lock distribuído (ShedLock, advisory lock do Postgres, etc.) | AD-003 trava monolito de instância única no v1 — não há múltiplas réplicas do bot rodando ao mesmo tempo, então o único cenário real de overlap é o mesmo processo disparando um novo ciclo antes do anterior terminar (ciclo demorado: muitos canais/produtos, latência de API externa). Um `AtomicBoolean` é atômico, sem espera, e resolve exatamente esse cenário sem a complexidade operacional de uma lib de lock distribuído que só se justificaria com múltiplas instâncias do bot — decisão simétrica a AD-002 (sem Redis) e AD-003 (sem infraestrutura de coordenação distribuída) |
...
4. ~~Esta feature também precisa **editar** `PromotionDetectionService`... para o novo retorno `List<CandidatoPromocaoDTO>`~~ — **obsoleto (correção registrada em 2026-09-10, revisão pré-Tasks)**: confirmado no código real (`PromotionDetectionService.java:27`) que `buscarCandidatosElegiveis()` já nasceu retornando `List<CandidatoPromocaoDTO>` na implementação original de `scraping-coleta` (commit `d37b5b0`) — a assinatura já reflete a decisão registrada aqui desde o início, provavelmente porque a revisão que motivou este item (feita durante o Design de `enriquecimento-conteudo`) chegou antes da implementação de `scraping-coleta` ter sido escrita. **Nenhuma task desta feature precisa tocar `PromotionDetectionService`.**
