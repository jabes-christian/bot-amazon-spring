...
- **Phase / Task**: Execute de `scraping-coleta`, próxima ação é T1 (Flyway+Testcontainers+migration inicial). Ordem de execução confirmada: scraping-coleta → enriquecimento-conteudo → canais-disparo → operacao-docker. Estratégia de teste confirmada: unit (JUnit Jupiter+Mockito+AssertJ via `spring-boot-starter-test`) para regra de negócio, integration via Testcontainers Postgres para repository/Flyway, sem e2e (app headless). Sub-agentes: usuário optou por rodar as 12 tasks sequencialmente com um agente só nesta 1ª feature (mesmo o packing em 2 batches sendo tecnicamente possível — T11/T12 dependem de T3-T6, então os batches não rodariam em paralelo de qualquer forma; o modelo do skill nunca paraleliza batches, só distribui o orçamento de contexto entre workers) — reavaliar sub-agentes numa próxima feature.
- **Completed**: PRD (`.specs/PRD.md`); AD-001..AD-009; `spec.md` das 4 features (SCRAPE-01..17, ENRICH-01..16, DISPATCH-01..26, OPS-01..19); `pom.xml` sem `spring-boot-starter-cache`/`-test` (AD-008); os 3 `design.md` (scraping-coleta, enriquecimento-conteudo, canais-disparo); `.specs/features/scraping-coleta/tasks.md` (12 tasks, 4 fases, validado por `validate_tasks.py` — 0 erros)
- **In-progress**: nenhum arquivo em edição (prestes a iniciar T1)
- **Next step**: Implementar T1 (pom.xml + application.properties + migration V1 + smoke test), rodar o gate (`mvn clean verify`), apresentar resultado + mensagem de commit sugerida ao usuário, aguardar confirmação de commit manual antes de avançar para T2
- **Blockers**: none
- **Regra de processo confirmada pelo usuário (Execute)**: nunca rodar comandos git. Ao final de cada task: apresentar gate check + done-when cumprido + mensagem de commit sugerida; aguardar o usuário confirmar que o commit foi feito manualmente antes de seguir para a task seguinte. Nunca avançar sem essa confirmação explícita.
- **Uncommitted files**: `.specs/PRD.md`, `.specs/STATE.md`, `.specs/features/*/spec.md`, `.specs/features/*/design.md`, `.specs/features/scraping-coleta/tasks.md`, `pom.xml` (usuário faz commit manualmente)
