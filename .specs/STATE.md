...
- **Feature**: `scraping-coleta` — **Done** (15/15 tasks, Verifier PASS iteração 2). `enriquecimento-conteudo` — **Done** (8/8 tasks, Verifier PASS iteração 3, 16/16 ACs, gate 65/65, sensor 2/2 mortos — `validation.md`). Ordem confirmada: scraping-coleta → enriquecimento-conteudo → canais-disparo → operacao-docker. **Feature atual: `canais-disparo`** — Tasks aprovada pelo usuário, Execute iniciando.
- **Phase / Task**: `canais-disparo` em **Execute**, prestes a iniciar **T1** (migration V3 — tabelas `channel`/`dispatch_history` + seed de config + extensão do smoke test).
- **Próxima ação concreta**: implementar T1 conforme `tasks.md`, rodar o gate (`mvn clean verify`), apresentar resultado + mensagem de commit sugerida, aguardar confirmação do usuário antes de seguir para T2.
- **Completed**: PRD (`.specs/PRD.md`); AD-001..AD-009; `spec.md` das 4 features (SCRAPE-01..17, ENRICH-01..16 — 16/16 Verified, DISPATCH-01..26, OPS-01..19); `pom.xml` sem `spring-boot-starter-cache`/`-test` (AD-008); os 3 `design.md` (scraping-coleta, enriquecimento-conteudo, canais-disparo); **`scraping-coleta` inteira: `tasks.md` (15 tasks) + implementação completa (47+ testes) + `validation.md` (PASS)**; **`enriquecimento-conteudo` inteira: `tasks.md` (8 tasks) + implementação completa (65 testes) + `validation.md` (PASS, iteração 3)**; **`canais-disparo`: `design.md` revisado (nota de atenção `DisparoService`, decisão de lock `AtomicBoolean` para DISPATCH-08, risco Evolution API 201+PENDING documentado) + `tasks.md` (9 tasks, 4 fases, aprovado pelo usuário, `validate_tasks.py` 0 erros/12 warnings pré-justificados) — ambos commitados e mergeados em `main` (`3c89b0c`).**
- **In-progress**: T1 de `canais-disparo` (migration V3) — nenhum arquivo ainda modificado nesta task.
...
- **Uncommitted files**: nenhum — working tree limpo. Último commit: `3c89b0c` (design.md revisado + tasks.md de `canais-disparo`), na branch atual, já mergeado em `main`.
- **Branch**: `tasks/canais-disparo` (branch atual)
