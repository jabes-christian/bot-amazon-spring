...
- **Feature**: `enriquecimento-conteudo` — Fase 3 (Design)
- **Phase / Task**: `design.md` de `scraping-coleta` aprovado. `design.md` de `enriquecimento-conteudo` escrito (EnriquecimentoService/CopyGenerationService/BannerImageService/ChatModel via LangChain4j+OpenRouter, timeout aplicado manualmente via ConfigService+virtual threads para honrar AD-009) — aguardando aprovação antes de seguir para `canais-disparo`
- **Completed**: PRD (`.specs/PRD.md`); AD-001..AD-009; `spec.md` das 4 features (SCRAPE-01..17, ENRICH-01..16, DISPATCH-01..26, OPS-01..19); `pom.xml` sem `spring-boot-starter-cache`/`-test` (AD-008); `.specs/features/scraping-coleta/design.md` (aprovado); `.specs/features/enriquecimento-conteudo/design.md` (aguardando aprovação — sem tabela nova, só +3 chaves em `app_config` e +1 método em `PriceHistoryRepository`)
- **In-progress**: nenhum arquivo em edição
- **Next step**: Após aprovação do design de `enriquecimento-conteudo`, seguir para Design de `canais-disparo` (última das 3 com design.md formal) — reusar `app_config`/`ConfigService` (AD-009) para janela de dedup/teto por canal/intervalo entre envios, e honrar os 2 contratos que `canais-disparo` precisa implementar: chamar `EnriquecimentoService.enriquecer(produto)` uma vez por produto antes do loop de canais, e chamar `BannerImageService.removerBanner(path)` após concluir todos os canais do produto. Só então Tasks — Flyway entra no `pom.xml` na Tasks de `scraping-coleta` (1ª migration), `enriquecimento-conteudo` adiciona uma 2ª migration só de seed em `app_config`
- **Blockers**: none
- **Uncommitted files**: `.specs/PRD.md`, `.specs/STATE.md`, `.specs/features/*/spec.md`, `.specs/features/scraping-coleta/design.md`, `.specs/features/enriquecimento-conteudo/design.md`, `pom.xml` (usuário faz commit manualmente)
