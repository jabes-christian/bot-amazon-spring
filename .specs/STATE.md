...
### AD-010
- **Decision**: (1) `OPENROUTER_MODEL` é fixado como `inclusionai/ling-3.0-flash-vl:free` ("Ling 3.0 Flash VL", via OpenRouter), não mais o Free Models Router genérico usado no fechamento original de `enriquecimento-conteudo`. (2) Telegram (`parse_mode=MarkdownV2`) e WhatsApp (Evolution API) usam sintaxe de ênfase de caractere único (`*negrito*`, `~tachado~`) — diferente do Markdown padrão/CommonMark que os LLMs produzem (`**negrito**`, `~~tachado~~`); `TelegramChannelSender`/`WhatsAppChannelSender` convertem essa sintaxe por canal antes do envio, além de o Telegram escapar os demais caracteres especiais do MarkdownV2 (nunca a URL do link de afiliado).
- **Reason**: (1) Teste manual comparando múltiplos modelos free no OpenRouter Playground com produtos e prompts reais — outros modelos do pool free testados (DeepSeek, LFM2.5, Nemotron) vazavam um artefato de controle tipo `<CPA_DONE>` no final da copy gerada, ou não respondiam dentro do timeout de 15s (`enriquecimento.llm-timeout-segundos`); o "Ling 3.0 Flash VL" não apresentou nenhum dos dois problemas. (2) Pesquisa na documentação oficial da Bot API do Telegram (via context7, `core.telegram.org/bots/api`) revelou que o MarkdownV2 nativo do Telegram usa tachado com um único `~` (o Markdown legado nem suporta tachado) e negrito com um único `*` — apenas escapar os caracteres especiais do texto gerado pelo LLM, como se supunha antes da pesquisa, não bastaria: o `**`/`~~` literal do LLM chegaria sem conversão e o Telegram exibiria os marcadores duplicados de forma literal (mesmo problema que motivou a conversão já prevista para o WhatsApp). Achado não estava documentado em nenhum `design.md` anterior porque a formatação de copy por canal não fazia parte do escopo original de `canais-disparo`.
- **Trade-off**: (1) Modelo fixo e específico em vez do roteamento automático do Free Models Router — menos resiliente a uma eventual descontinuação/instabilidade desse modelo específico no OpenRouter, mas elimina o vazamento de artefato de controle e a variabilidade de latência entre modelos do pool free observada nos testes manuais. (2) A copy gerada uma única vez por produto (decisão existente, não alterada) precisa ser convertida por canal antes do envio — pequeno custo de processamento e mais uma responsabilidade em cada `ChannelSender`, em vez de assumir que o Markdown do LLM funciona igual em todo canal; a conversão preserva deliberadamente a URL do link de afiliado sem escapar (evita `amazon\.com\.br` visível e preserva o auto-link do Telegram).
- **Scope**: `CopyGenerationService` (prompt de geração de copy) e qualquer `ChannelSender` (hoje `TelegramChannelSender`/`WhatsAppChannelSender`, e qualquer canal futuro) — referência para não repetir a pesquisa de sintaxe de ênfase por canal nem reverter a conversão por engano. Não reabre `enriquecimento-conteudo` nem `canais-disparo`, já fechadas com Verifier PASS; decisão registrada fora do ciclo formal de feature, via testes manuais e fixes pontuais em branch própria (`fix/formatacao-copy-por-canal`).
- **Date**: 2026-09-14
- **Status**: active
